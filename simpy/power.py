...
def get_ndarray(Data_Vec):
    """ copy C++ class Data_Vec<double, N> into numpy array """
    dim = Data_Vec.dim()
    data = [[x for x in Data_Vec[i]] for i in xrange(dim)]
    return np.array(data)

def get_Data_vec(data):
    """ copy 2D data 3xN into C++ class Data_Vec<double, 3> """
    dim = len(data)
    size = len(data[0])
    if dim == 2:
        ...
        raise IndexError("only Data_Vec<double, dim> of 'dim' 2 or 3 supported")
    for j in xrange(dim):
